...
| 型 | 構成 |
|---|---|
| 体験速報 | 短い口語＋結果を先出し＋「いいね多かったら詳細出す」。最も反応が取れる |
| 番号リスト | 「どうぞパクってください」→ 1〜7の実用項目 → 締めの一言 |
| 会話劇 | セリフの応酬で実話を再現し、最後にツッコミで落とす |
| 長文ストーリー | 権威付け → 共感 → 失敗の痛み →「でも逆に言うと」→ プロフィール誘導 |
...
"post_schedule": {
  "10": ["today"],
  "13": ["info", "aruaru", "cheatsheet", "info", "aruaru", "lineup", "price"],
