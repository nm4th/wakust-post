...
```bash
python wakust_threads.py --template pinned
```
...
1名だけの駅は並べても情報にならないので、2名以上に絞っています。
本文が480字を超える場合はエリアだけの表示に切り替わります。

**投稿したあと、Threadsアプリでピン留めしてください**（APIにピン留め機能は
ありません）。在籍が大きく変わったら作り直して貼り替えます。
`pinned` はローテーションには入れていないので、自動投稿されることはありません。

全テンプレートのCTAは「本命は固定ポストにまとめてます📌」に統一してあります
（`threads.profile_cta`）。
