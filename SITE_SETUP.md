...
**直近の投稿で推した場所が先頭に来ます。** 13:00に池袋の `pickup` を出したら、
固定ポストの本命も池袋が先頭になり、「池袋の本命は書いてない」→ 固定ポスト、の
往復が噛み合います。
...
貼り替えは `Wakust Threads Post` の最後のステップで毎回走ります（1日4〜5回）。
返信は投稿数の上限にカウントされないので、頻度を上げても問題ありません。
古い返信は削除してから新しいものを付けるので溜まりません。削除に失敗しても
新しい返信は付きます（その場合だけ手動で消してください）。

`Wakust Threads Pinned Refresh`（週1）は取りこぼし用の保険です。

### ピン留めは自動化できません

**Threads API にピン留めのエンドポイントはありません。** ただし
**ピン留めが必要なのは最初の1回だけ**です。以後は本体をさわらず返信だけを
差し替えるので、ピンは外れません。
